...
class HomeController extends Controller
{
    public function dashboard()
    {
        ...
        $data['TotalDeposit'] = Deposit::where('payment_status', 1)->sum('final_amount');
        $data['TotalWithdraw'] = Withdraw::where('status', 1)->sum('withdraw_amount');

        $data['TodayDeposit'] = Deposit::where('payment_status', 1)
            ->whereDate('created_at', Carbon::today())
            ->sum('final_amount');
        $data['TodayWithdraw'] = Withdraw::where('status', 1)
            ->whereDate('created_at', Carbon::today())
            ->sum('withdraw_amount');
