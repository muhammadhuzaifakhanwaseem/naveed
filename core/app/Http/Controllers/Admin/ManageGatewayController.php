...
class ManageGatewayController extends Controller
{
    ...
    public function depositLog(Request $request)
    {
        ...
        $manuals = Deposit::query()->where('payment_type', 0)->with('user');

        // **Search by Email**
        if ($request->has('email') && !empty($request->email)) {
            $email = $request->email;
            $manuals->whereHas('user', function ($query) use ($email) {
                $query->where('email', 'LIKE', "%{$email}%");
            });
        }

        $data['manuals'] = $manuals->latest()->paginate(10);
    ...
    public function depositDetails(Request $request)
{
    ...
    $manual = Deposit::where('transaction_id', $request->trx)->with('user')->firstOrFail();
