...
{
    $pageTitle = "Playstore";
    return view($this->template . 'pages.playstore', compact('pageTitle'));
}
// Certificate
public function certificate()
{
    $pageTitle = "certificate";
    return view($this->template . 'pages.certificate', compact('pageTitle'));
}
    ...
    public function investmentUsingBalannce(Request $request)
    {
        ...
        $CheckUserPayments = Payment::where('user_id', auth()->id())
            ->where('plan_id', 48)
            ->count();

        if ($plan_id->id == 48 && $CheckUserPayments >= 2) {
            return redirect()->back()->with('error', 'You can’t invest more than 2 times in this plan.');
        }
